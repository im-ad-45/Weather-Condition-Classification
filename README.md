...
## Results
Since this notebook pulls the live 7-day forecast at run time, the exact numbers shift slightly each time it's run. Run `Assignment-6.ipynb` top to bottom and record the values printed under Task 4 here, for example:
...
In testing, accuracy typically comes out very high — see the Conclusion below for why that's expected rather than a sign of an unusually strong model.
